...
let count = 0;
const MAX_RANDOM = 4; //quantos eventos no maximo entram na fila
let atendimentoInterval = null;
let filaInterval = null;
//let numero_aleatorio = Math.floor(Math.random() * Math.random());
...
export default class Tds extends Component {
    ...
    _geraNumeroAleatorio = (numero_aleatorio) => {
          
        return numero_aleatorio = Math.floor(Math.random() * MAX_RANDOM);
      }
    ...
    _gerenciarFila = () => {
        filaInterval = setInterval(() => {
            ...
            if (!paused) {
                ...
                let random = this._geraNumeroAleatorio();
                    ...
                    { 
                        this.state.fila.map((v, k) => (
                            ...
                            <Cliente
                                ...
                                top='0%'
                                left={ '0%' }
                                text={ 'C' + this._geraNumeroAleatorio()}
                            />
                        ))
                    }
